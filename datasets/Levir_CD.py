...
def spatial_transform(img1, img2, mask):
    aug = Compose([
        VerticalFlip(p=0.5),
        HorizontalFlip(p=0.5),
        RandomRotate90(p=0.5),
        #ShiftScaleRotate(shift_limit=0., scale_limit=[0.5,1.0], rotate_limit=0., p=0.5),
...
class RS(data.Dataset):
    def __init__(self, mode, normalize=False, random_crop=False, crop_nums=6, sliding_crop=False, crop_size=512, random_flip=False):
        self.mode = mode
    ...
    def __getitem__(self, idx):
        ...
        if self.mode=='train':        
           ...
           data_A, data_B, label = spatial_transform(data_A, data_B, label)
           #data_A, data_B = color_transform(data_A), color_transform(data_B)
