...
import sys
import math
sys.path.append(os.path.abspath("./RADIO"))
import torch
...
class Lora_Radio_model(nn.Module):
    def __init__(self, r=8, lora_layer=None): #lora_layer=[8,9,10,11]
        super(Lora_Radio_model, self).__init__()        
        assert r > 0
        # create for storage, then we can init them or load weights
        self.w_As = []  # These are linear layers
        self.w_Bs = []
                
        model = radio_model(version='radio_v2.5-b', force_reload=True).cuda()
        if lora_layer:
